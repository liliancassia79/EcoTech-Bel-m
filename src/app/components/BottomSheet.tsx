...
import { ChevronUp, ChevronDown, Leaf, Droplet, MapPin, X, Info } from "lucide-react";
...
export default function BottomSheet({ locations }: BottomSheetProps) {
  ...
  return (
    <>
      ...
      {/* Floating reopen button when sheet is closed */}
      <AnimatePresence>
        {!isVisible && (
          <motion.button
            key="reopen-fab"
            ...
            className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 bg-gradient-to-r from-[var(--eco-amazon)] to-[var(--eco-amazon-dark)] text-white px-4 py-2.5 rounded-full shadow-2xl border-2 border-white/30 hover:shadow-emerald-500/40 transition"
            aria-label="Reabrir informações ambientais"
            title="Mostrar informações"
          >
            <Info className="w-4 h-4" />
            <span className="text-xs font-bold">Info ambiental</span>
          </motion.button>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            animate={{ y: isExpanded ? -280 : 0, opacity: 1 }}
          ...
          >
            <div className="bg-white/98 dark:bg-gray-900/98 backdrop-blur-md rounded-t-3xl shadow-2xl border-t-4 border-[var(--eco-amazon)]">
              ...
              <button
                onClick={handleClose}
                className="absolute top-2 right-3 z-10 p-1.5 rounded-full bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 transition shadow-sm"
                ...
                {!isExpanded && (
                  ...
                  >
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      <strong className="text-[var(--eco-amazon)] dark:text-green-400">
                        {locations.length} pontos certificados
                      </strong>{" "}
                      • De Icoaraci ao Ver-o-Peso
                    </p>
                  </motion.div>
                )}
              </div>
